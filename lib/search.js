...
// This is used to ignore search differences such as accents
...
Search.prototype.search = function(group, string, limit, cb) {
  ...
  var str = stemmer(removeDiacritics(string.toLowerCase()));
  var zrange = '[' + str;
 ...
 * If options are passed with {strategy: noTokens}, the string is not broken
 * up into individual tokens. "The fox jumps over the lazy dog" is indexed as is.
 * This strategy will also index "fox jumps over the lazy dog", "jumps over
...
Search.prototype.index = function (group, doc, keys, options, cb) {
  ...
  function _index(str, cb) {
    ...
    if (options && options.strategy === 'noTokens') {
      ...
      var ___cb = _.after(count, cb);
      for (i = 0; i < count ; i++) {
        // Commonalize words and accents
        var str = stemmer(removeDiacritics(str.toLowerCase()));
        self.client.zadd(group + '-search', 0, str + '::' + doc._id, ___cb);
        indexed++;
      ...
      _.each(str, function (s) {
        if (s === '') return ___cb();
        var s = stemmer(removeDiacritics(s.toLowerCase()));
        self.client.zadd(group + '-search', 0, s + '::' + doc._id, ___cb);
        indexed++;
      });
    }
  }
